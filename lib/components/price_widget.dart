...
class PriceWidget extends StatelessWidget {
  ...
  PriceWidget({
    ...
    this.isFixesService = false,
    this.isDailyService = false,
    this.decimalPoint, this.hourlyTextStyle,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        Row(
          children: [
            if (isHourlyService) Text(
              '${price.validate().toPriceFormat()}/${languages.lblHr}',
              style: hourlyTextStyle ?? secondaryTextStyle(
                color: hourlyTextColor,
                size: 14,
              ),
            ) else if (isFixesService) Text(
              '${price.validate().toPriceFormat()}/${languages.lblFixed}',
              style:  hourlyTextStyle ?? secondaryTextStyle(
                color: hourlyTextColor,
                size: 14,
              ),
            ) else if (isDailyService) Text(
              '${price.validate().toPriceFormat()}/${languages.lblDay}',
              style: hourlyTextStyle ?? secondaryTextStyle(
                color: hourlyTextColor,
                size: 14,
              ),
            ) else if(isFreeService) Text(
              languages.lblFree,
              style: _textStyle(),
            )
            else Text(
