# Google Play 7-inch tablet screenshots: 16:9 or 9:16, 320-3840 px each side, up to 8 MB each.
# Output: 9:16 portrait (1080 x 1920 px) - same spec as phone, for tablet section.
...
$outDir = Join-Path $assetsDir "play_store_screenshots_7inch"
if (-not (Test-Path $outDir)) { New-Item -ItemType Directory -Path $outDir -Force | Out-Null }

# 9:16 portrait (1080 x 1920) - within 320-3840 and under 8 MB
$targetW = 1080
$targetH = 1920
...
foreach ($src in $sources) {
    ...
    $outPath = Join-Path $outDir "play_store_tablet_screenshot_$index.jpg"
    ...
    try {
        ...
        Write-Host "Created: play_store_tablet_screenshot_$index.jpg ($targetW x $targetH, $sizeMB MB)"
        $index++
    } finally {
        $img.Dispose()
    }
}
Write-Host "Done. 7-inch tablet screenshots in: $outDir"
Write-Host "Upload up to 8 of these in Play Console (Store listing > 7-inch tablet screenshots). 9:16, 1080x1920, under 8 MB each."
